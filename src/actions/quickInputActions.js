import { applyBaseLabelToItems, applyExpiryToItems, applyGroupToItems, makeCard, nowIso, parsePasteItems, parseTags, sortedItems, stampLine } from "../domain.js";
...
export function createQuickInputActions(ctx) {
  ...
  function submitQuickInput(fields) {
    ...
    const items = applyGroupToItems(applyExpiryToItems(applyBaseLabelToItems(parsePasteItems(raw, {
      splitMode: fields.quickSplitMode,
      splitPattern: fields.quickSplitPattern
    }), fields.quickBaseLabel), fields.quickExpiresAt), fields.quickGroup);
    if (!items.length) {
      notify("붙여넣을 줄이 없습니다");
      return false;
    }
  ...
  function quickPaste(form) {
    ...
    submitQuickInput({
      ...
      quickGroup: formData.get("quickGroup"),
